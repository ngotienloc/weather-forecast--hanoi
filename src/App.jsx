...
export default function App() {
  ...
  const fetchWeatherData = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch("https://api.open-meteo.com/v1/forecast?latitude=21.0274&longitude=105.8988&current_weather=true&hourly=temperature_2m,apparent_temperature,visibility,precipitation_probability,weathercode,windspeed_10m&daily=temperature_2m_max,temperature_2m_min,weathercode,precipitation_sum&timezone=Asia%2FHo_Chi_Minh&forecast_days=8");
    ...
    if (loading) {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '80%', justifyContent: 'center', alignItems: 'center' }}>
          ...
          <div style={{ color: 'var(--text-muted)', fontSize: '13px' }}>Đang lấy thông tin thời tiết Aeon Mall Long Biên...</div>
          <style>{`
            @keyframes spin {
